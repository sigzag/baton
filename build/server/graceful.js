...
function graceful(server, isReady, down) {
  ...
  function shutDown() {
    ...
    Promise.all([down(), new Promise(resolve => server.close(resolve))]).then(() => {
  ...
  function graceful(req, res, next) {
    if (shuttingDown || !isReady()) throw new _errors.ServiceUnavailable();
    next();
  }
