...
function getNodeType(name, resolveType) {
	if (!objectTypes[name]) {
		const nodeType = resolveType
			? new GraphQLInterfaceType({ name, resolveType: resolveType(getNodeType) })
			: new GraphQLObjectType({ name, interfaces: [], fields: {} });
		objectTypes[name] = nodeType;
		connectionDefinition[name] = connectionDefinitions({ nodeType });
	}
