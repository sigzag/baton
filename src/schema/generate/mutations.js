...
function updateGraph(type, field, { filename }) {
	const Type = capitalize(type);
...
function getFields(type) {
	return typeof type._typeConfig.fields === 'function'
		? type._typeConfig.fields()
		: type._typeConfig.fields;
}
function fields(type) {
	return map(getFields(type), (field, name) => {
		if (/Connection$/.test(field.type._typeConfig && field.type._typeConfig.name))
			return `${name}(first: 2147483647) { edges { node { id } } }`;
		
		const subType = field.type instanceof GraphQLList
			? field.type.ofType
			: field.type;
		if (subType instanceof GraphQLObjectType || subType instanceof GraphQLInterfaceType) {
			if (getFields(subType).hasOwnProperty('id'))
				return `${name} { id }`;
			else
				return `${name} { ${fields(subType)} }`;
		}
...
export default function(schema, options) {
	if (!options || !options.hasOwnProperty('filename'))
		throw new Error('Must include "filename" option because relay dumb af, but we like it anyway cuz it still has a nice body (of work)');
...
function cleanNodeInput(value) {
	...
	for (let objectType of values(schema.objectTypes)) {
		...
		output += map(getFields(objectType), ({ diff }, name) => diff
			? updateGraph(objectType.name, name, options)
			: '').join('');
	}
	return output;
}